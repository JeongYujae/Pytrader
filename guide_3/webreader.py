...
def get_3year_treasury():
    url = "http://www.index.go.kr/strata/jsp/showStblGams3.jsp?stts_cd=107301&amp;idx_cd=1073&amp;freq=Y&amp;period=1997:2021"
    html = requests.get(url, verify=False).text
    soup = BeautifulSoup(html, 'html5lib')
...
def get_estimated_dividend_yield(code):#현금배당수익률 파싱 함수(최근 결산 수정 base)
...
def get_current_3year_treasury(): #현금배당수익률 파싱 함수(일별 시세 base)
...
def get_previous_dividend_yield(code): #과거 5년치 현금배당수익률
    dividend_yield = get_financial_statements(code)    
